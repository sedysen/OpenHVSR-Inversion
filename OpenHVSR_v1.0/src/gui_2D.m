...
%
MDLS       = {};                                % Subsurface Initial models [vp  vs  rho  h  Qp  Qs] - > [VPprofile,VSprofile,ROprofile,Hprofile,QPprofile,QSprofile]
...
curve_plotmode         = 1;
%%    Initial Values for the main sacle
...
%%   Malagnini_1997_BSSA87__Final  Table 2 Table 2 (NO VP)
%    computed 
Malagnini_1997_BSSA87__Final_tab2 = [ ...
3  0  140.0; ...
5  0  139.1; ...
5  0  134.6; ...
5  0  244.7; ...
...
%%   Malagnini_1997_BSSA87__ Cross-hole
H3=[ ...
5.0; ...
7.5; ...
10.0; ...
...
414; ...
432];
Malagnini_1997_BSSA87__crosshole = [H3, Vp3, Vs3];
clear H3 Vp3 Vs3
%% vp  vs  rho  h  Qp  Qs
a1 = Malagnini_1997_BSSA87__Final_tab2 
REFERENCE_MODEL_dH       = [ a1(:,2), a1(:,3), 0*a1(:,1), a1(:,1), 0*a1(:,1), 0*a1(:,1)];
a2 = Malagnini_1997_BSSA87__crosshole
REFERENCE_MODEL_zpoints  = [ a2(:,2), a2(:,3), 0*a2(:,1), -a2(:,1), 0*a2(:,1), 0*a2(:,1)];
...
%
uimenu(h0,'Label','Save Model as .txt set','Callback',{@Menu_Model_Save_txt_set}, ...
'Separator','on');
uimenu(h0,'Label','Save Curves','Callback',{@Menu_Curve_Save_txt_set});
uimenu(h0,'Label','Save as new project(TEMP)','Callback',{@Menu_Save_as_newproject_set});
%
...
uimenu(h52,'Label','Interpolation','Callback',{@Menu_view_profile_interp});
%%        Smooting
...
%%    About
h10  = uimenu(h_gui,'Label','About TMI');
...
%objx = 0.01 + [0, objw(1), (sum(objw(1:2)) + gapx), (sum(objw(1:3)) + gapx) , (sum(objw(1:4)) + gapx)];
...
%%      Files        
    function Menu_Project_Load(~,~,~)
        [file,path] = uigetfile('*.m','Load Project Script(.m)',strcat(working_folder,last_project_name) );
        if(file ~= 0)
    ...
    function Menu_Curve_Save_txt_set(~,~,~)
        ...
        if(folder_name)
            ...
            if Nsurveys>0
                for i = 1:Nsurveys
                    [p,s,ext]=fileparts(SURVEYS{i,2});
                    fname = strcat(folder_name,'/',s,'_sinthetic_hvsr.txt');
                    dd = [modl_FDAT{i,1},modl_FDAT{i,2}];
                    save(fname,'dd','-ascii'); 
                end
            end 
        end
    ...
    function Menu_Save_as_newproject_set(~,~,~)
        ...
        if(folder_name)
            ...
            if Nsurveys>0
                for i = 1:Nsurveys
%                     [p,s,ext]=fileparts(SURVEYS{i,2});
%                     fname = strcat(folder_name,'/',s,'_sinthetic.txt');
%                     dd = [modl_FDAT{i,1},modl_FDAT{i,2}];
                    % data
                    fname = strcat(folder_name,'/',SURVEYS{i,2});
                    dd = [FDAT{i,1},FDAT{i,2}];
                    save(fname,'dd','-ascii');
                    
                    
                    is = num2str(i);
                    stringline = strcat('SURVEYS{',is,',1} = [',num2str(SURVEYS{i,1}(1)),', ',num2str(SURVEYS{i,1}(2)),', ',num2str(SURVEYS{i,1}(3)),'];\n');
    ...
    function Menu_Save_elaboration(~,~,~)
        [file,path] =  uiputfile('*.mat','Save elaboration', strcat(working_folder,'Elaboration.mat'));
        ...
        if(file ~= 0)
            ...
            datname = strcat(path,file);
            save(datname);
        end
    end
    function Menu_Load_elaboration(~,~,~)
        [file,path] = uigetfile('*.mat','Resume Elaboration',strcat(working_folder,'/Elaboration.mat'));
        datname = strcat(path,file);
        if(file ~= 0)
            ...
            %id                              = BIN.id;
            min_vs                          = BIN.min_vs;
            min_vp_vs_ratio                 = BIN.min_vp_vs_ratio;    
            max_vp_vs_ratio                 = BIN.max_vp_vs_ratio;
            min_ro                          = BIN.min_ro;
            max_ro                          = BIN.max_ro;
            min_H                           = BIN.min_H;
            max_H                           = BIN.max_H; 
            min_qs                          = BIN.min_qs; 
            max_qs                          = BIN.max_qs; 
            min_qp_qs_ratio                 = BIN.min_qp_qs_ratio;
            max_qp_qs_ratio                 = BIN.max_qp_qs_ratio;
            %
            HQMagnitude                     = BIN.HQMagnitude;
            HQEpicDistance                  = BIN.HQEpicDistance;
            HQFocalDepth                    = BIN.HQFocalDepth; 
            HQRockRatio                     = BIN.HQRockRatio;
            %
            %working_folder                  = BIN.
            SURVEYS                         = BIN.SURVEYS;
            MODELS                          = BIN.MODELS;
            FDAT                            = BIN.FDAT;
            DISCARDS                        = BIN.DISCARDS;
            modl_FDAT                       = BIN.modl_FDAT;
            last_FDAT                       = BIN.last_FDAT;
            best_FDAT                       = BIN.best_FDAT;
            MDLS                            = BIN.MDLS;
            last_MDLS                       = BIN.last_MDLS;
            prev_MDLS                       = BIN.prev_MDLS;
            prev_BEST_SINGLE_MODELS         = BIN.prev_BEST_SINGLE_MODELS;
            prev_best_FDAT                  = BIN.prev_best_FDAT;
            prev_BEST_SINGLE_MISFIT         = BIN.prev_BEST_SINGLE_MISFIT;
            prev_single_it                  = BIN.prev_single_it;
            weight_curv                     = BIN.weight_curv;
            CW                              = BIN.CW;
            weight_dpth                     = BIN.weight_dpth;
            DW                              = BIN.DW;
            DL                              = BIN.DL;
            if isfield(BIN,'LKT');       LKT=       BIN.LKT; end
            if isfield(BIN,'RCT');       RCT=       BIN.RCT; end
            
            TO_INVERT                       = BIN.TO_INVERT;
            NresultsToKeep                  = BIN.NresultsToKeep;
            global_inversion_step           = BIN.global_inversion_step;
            BEST_MISFITS                    = BIN.BEST_MISFITS;
            BEST_ENERGIES                   = BIN.BEST_ENERGIES;
            BEST_MODELS                     = BIN.BEST_MODELS;
            BEST_SINGLE_MISFIT              = BIN.BEST_SINGLE_MISFIT;
            BEST_SINGLE_MODELS              = BIN.BEST_SINGLE_MODELS;
            last_single_FDAT                = BIN.last_single_FDAT;
            init_single_FDAT                = BIN.init_single_FDAT;
            last_single_MDL                 = BIN.last_single_MDL;
            BREAKS                          = BIN.BREAKS;
            independent_optimiazation_cicles= BIN.independent_optimiazation_cicles;
            STORED_RESULTS_1d_misfit        = BIN.STORED_RESULTS_1d_misfit;
            STORED_RESULTS_2d_misfit        = BIN.STORED_RESULTS_2d_misfit;
            STORED_RESULTS_2d_misfit_profile= BIN.STORED_RESULTS_2d_misfit_profile;
            STORED_1d_vp_fits               = BIN.STORED_1d_vp_fits;
            STORED_1d_vs_fits               = BIN.STORED_1d_vs_fits;
            STORED_1d_ro_fits               = BIN.STORED_1d_ro_fits;
            STORED_1d_hh_fits               = BIN.STORED_1d_hh_fits;
            STORED_1d_qp_fits               = BIN.STORED_1d_qp_fits;
            STORED_1d_qs_fits               = BIN.STORED_1d_qs_fits;
            STORED_1d_daf_fits              = BIN.STORED_1d_daf_fits;
            STORED_2d_vp_fits               = BIN.STORED_2d_vp_fits;
            STORED_2d_vs_fits               = BIN.STORED_2d_vs_fits;
            STORED_2d_ro_fits               = BIN.STORED_2d_ro_fits;
            STORED_2d_hh_fits               = BIN.STORED_2d_hh_fits;
            STORED_2d_qp_fits               = BIN.STORED_2d_qp_fits;
            STORED_2d_qs_fits               = BIN.STORED_2d_qs_fits;
            STORED_2d_daf_fits              = BIN.STORED_2d_daf_fits;
            data_1d_to_show                 = BIN.data_1d_to_show;
            %property_1d_to_show: 0
            %property_2d_to_show: 0
            Zprofile                        = BIN.Zprofile;
            VPprofile                       = BIN.VPprofile;
            VSprofile                       = BIN.VSprofile;
            ROprofile                       = BIN.ROprofile;
            QPprofile                       = BIN.QPprofile;
            QSprofile                       = BIN.QSprofile;
            zlevels                         = BIN.zlevels;
            xpositions                      = BIN.xpositions;
            bedrock                         = BIN.bedrock;
            datafile_columns                = BIN.datafile_columns;
            datafile_separator              = BIN.datafile_separator;
            n_depth_levels                  = BIN.n_depth_levels;
            smoothing_strategy              = BIN.smoothing_strategy;
            smoothing_radius                = BIN.smoothing_radius;
            HQfreq                          = BIN.HQfreq;
            QHspec                          = BIN.QHspec;
            %conf_1d_to_show_x               = BIN.
            %conf_1d_to_show_y               = BIN.
            misfit_over_sumweight           = BIN.misfit_over_sumweight;
            ndegfree                        = BIN.ndegfree;
            lsmooth                         = BIN.lsmooth;
            nlvl                            = BIN.nlvl;
            %default_colormap                = BIN.
            nx_ticks                        = BIN.nx_ticks;
            nz_ticks                        = BIN.nz_ticks;
            %curve_weights_plotmode          = BIN.curve_weights_plotmode
            %depth_weights_plotmode          = BIN.depth_weights_plotmode
            %curve_plotmode                  = BIN.curve_plotmode
            main_scale                      = BIN.main_scale;
            ixmin_id                        = BIN.ixmin_id;
            ixmax_id                        = BIN.ixmax_id;
            view_min_scale                  = BIN.view_min_scale;
            view_max_scale                  = BIN.view_max_scale;
            %init_value__fref                = BIN.
            %init_value__dscale              = BIN.
            %init_value__min_scale           = BIN.
            %init_value__max_scale           = BIN.
            max_lat_dVp                     = BIN.max_lat_dVp;
            max_lat_dVs                     = BIN.max_lat_dVs;
            max_lat_dRo                     = BIN.max_lat_dRo;
            max_lat_dH                      = BIN.max_lat_dH;
            max_lat_dQp                     = BIN.max_lat_dQp;
            max_lat_dQs                     = BIN.max_lat_dQs;
            version                         = BIN.version;
...
%%      Settings
    function Menu_Settings_Setup(~,~,~)
        [min_vs, ...
         ...
         min_qs,max_qs, ...
         min_qp_qs_ratio,max_qp_qs_ratio, ...
         HQMagnitude, HQEpicDistance, HQFocalDepth,  HQRockRatio] = setup_manager( ...
          min_vs, ... 
          min_vp_vs_ratio, max_vp_vs_ratio, ...
          min_ro,max_ro, ...
          min_H,max_H, ...
...
%%      View
%%          interpolation
    function Menu_view_profile_interp(~,~,~)
        prompt = {'N of x ticks','N of z  ticks'};
        def = {num2str(nx_ticks), num2str(nz_ticks)};
        answer = inputdlg(prompt,'Set resolution of 2D profile',1,def);
        
        nx_ticks = str2double(answer{1});         
        nz_ticks = str2double(answer{2});
...
%%          smoothing
    function Menu_smoothing_strategy0_Callback(~,~,~)
        ...
        plot_2d_profile( h_gui,  hAx_2Dprof,property_2d_to_show);
    end
...
%                fprintf('%d) ENERGY[%f]    MISFIT[%f]  REGULARIZER[%f]\n',global_inversion_step,Energy,fullMisfit,Rfit);
            ...
            set(hObject,'Value',0)
            set(hObject,'String','START Inversion')
        else
            fprintf('No data was loaded. Please load a project.\n')
        end
    end
...
%             fref = str2double( get(h_fref_val,'String') ); 
            ...
            if(TO_INVERT(data_1d_to_show) == 1)%                               Assuming model is not locked  
                while (get(hObject,'Value') && (togo > 0) )
                    ...
                    %% ====================================================
%                     % FORWARD: HERAk's routine translated by Bignardi here ! 
%                     % as_Samuel(c,ro,h,q,ex,fref,f)  
%                     %  vp  vs  rho  h  Qp  Qs
%                     VP = last_single_MDL(:,1);
%                     VS = last_single_MDL(:,2);
%                     RO = last_single_MDL(:,3);
%                     HH = last_single_MDL(:,4);
...
%                     [DAF] = get_amplification_factor(x_vec,aswave);
%                     
%                     last_single_FDAT{1,2} = 0*main_scale;
%                     last_single_FDAT{1,2}(ixmin_id:ixmax_id,1) = ( interp1(x_vec, hvsr_teo, main_scale(ixmin_id:ixmax_id) ) ).';
%                   
            ...
            set(hObject,'Value',0)
            set(hObject,'String','Optimize Independently')
       else
            %fprintf('No data was loaded. Please load a project.\n')
...
%% TAB-4: =================================================
    function BT_refresh_2D_profile(~,~,~)
        refresh_2D_profile();
    end
    function BT_show_2D_profile(~,~,parameter_id)
        property_2d_to_show = parameter_id;
        plot_2d_profile( h_gui,  hAx_2Dprof,property_2d_to_show);
    end
%% TAB-5: =================================================
%% TAB-5, Panel-1
...
%             end
            ...
            if( ixmax_id>ixmin_id && ixmax_id>0)
                ...
                %% show simple modeled (last user pressed 'MODEL' button)
            ...
            if(~isempty(FDAT{data_1d_to_show,3}))
                lgnd = [lgnd; 'err. '];
                if( curve_plotmode == 0)
                    plot(hhdl, scale, show_data+FDAT{data_1d_to_show,3},'linewidth',0.5,'color',[0.5 0.5 0.5]);
                else
                    semilogx(hhdl, scale, show_data+FDAT{data_1d_to_show,3},'linewidth',0.5,'color',[0.5 0.5 0.5]);
                end
...
%         %nnx
    ...
    function spunta(handle_vector, property_value)
        % spunta for menus
        for i = 1:length(handle_vector)
            if i == property_value +1;
                set(handle_vector(i),'Checked','on');
            else
                set(handle_vector(i),'Checked','off');
            end
        end
    end
...
%                 end
    ...
    function RFit = Regularize()
        ...
        for mi = 2:nmodels
            if(BREAKS(mi-1) == 0)
                vp_part=sum( VPprofile(:,mi-1) - VPprofile(:,mi) );
                vs_part=sum( VSprofile(:,mi-1) - VSprofile(:,mi) );
                ro_part=sum( ROprofile(:,mi-1) - ROprofile(:,mi) );
                qp_part=sum( QPprofile(:,mi-1) - QPprofile(:,mi) );
                qs_part=sum( QSprofile(:,mi-1) - QSprofile(:,mi) );
            end
            
            RFit = RFit + ( (wro*ro_part)^2 + (wvp*vp_part)^2 + (wvs*vs_part)^2 + (wqp*qp_part)^2 + (wqs*qs_part)^2 );
        end
    end
...
%         loglog(xygraph(:,1),xygraph(:,2),'.-b')
    ...
    function refresh_2D_profile()
        ...
        Zprofile = zeros(length(zlevels), nmodels);
        VPprofile = Zprofile;
        VSprofile = Zprofile;
        ROprofile = Zprofile;
        QPprofile = Zprofile;
                ...
                for iz = 1:length(zlevels)
                    ...
                    if (btm < z) && (z <= top)
                        ...
                        if(l<nlay)
                            ...
                            QPprofile(iz,m) = temp(l-1,5);
                            QSprofile(iz,m) = temp(l-1,5);
                        end
                        %fprintf('      [%d]  %f  >>> %f\n',iz,z,Lrho)
                    end
                    if(z<=btm)
                        if(l == nlay)
                            ...
                            QPprofile(iz,m) = temp(nlay-1,5);
                            QSprofile(iz,m) = temp(nlay-1,5);
                        else
                            break;
                        end
    ...
    function plot_2d_profile(figure_handle, axes_handle, quantity)
        ...
        %  quantity
        %  vp  vs  rho  xhx  Qp  Qs
        if (isempty(ROprofile) || isempty(VPprofile) || isempty(QPprofile) || isempty(VSprofile) || isempty(QSprofile)); refresh_2D_profile(); end       
        if (quantity == 1); imagenorm( VPprofile,  xpositions, zlevels, nx_ticks,nz_ticks, smoothing_strategy,smoothing_radius); str='Vp'; end
        if (quantity == 2); imagenorm( VSprofile,  xpositions, zlevels, nx_ticks,nz_ticks, smoothing_strategy,smoothing_radius); str='Vs'; end
        if (quantity == 3); imagenorm( ROprofile,  xpositions, zlevels, nx_ticks,nz_ticks, smoothing_strategy,smoothing_radius); str='RO'; end
        if (quantity == 4); imagenorm( QPprofile,  xpositions, zlevels, nx_ticks,nz_ticks, smoothing_strategy,smoothing_radius); str='Qp'; end
        if (quantity == 5); imagenorm( QSprofile,  xpositions, zlevels, nx_ticks,nz_ticks, smoothing_strategy,smoothing_radius); str='Qs'; end
%    
		axis(axes_handle,'xy')
        hold(axes_handle,'on')
        ...
        for m = 1:size(MDLS,2)
            Z = modeldepths(m);
            X = SURVEYS{m,1}(1) * ones(length(Z),1);
            if(min(Z)<maxdepth); maxdepth = min(Z); end
            plot(axes_handle,  X,Z,'ow','LineWidth',3,'MarkerSize',5);
        end
        
        %% bedrock line
        plot(axes_handle,  bedrock(1,:),bedrock(3,:),'o-w','LineWidth',3,'MarkerSize',5);
        %% bedrock mask
        maskline = bedrock(3,:)-min(abs(bedrock(3,:)-maxdepth)); 
        area(bedrock(1,:), maskline,   'basevalue', maxdepth,  'FaceColor',[0 0 0]);%,shift*(-ii+1));% [0 0 1]
%min(bedrock(3,:))
    ...
    function plot_extern(~,~,idx)
        ...
        switch(idx) 
            ...
            case 6;% view model 2d
                if(property_2d_to_show>0); plot_2d_profile(hxten, hAxs, property_2d_to_show); end
    ...
    function funct_saveimage(~,~,~)
        [file,path] =  uiputfile({'*.eps';'*.jpg';'*.fig'},'Save image as', strcat(working_folder,'image.eps'));
