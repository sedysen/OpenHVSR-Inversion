...
r_reciprocicity = [];                          % station to station distances table
receiver_locations     = [];                          % stations locations
cutplanes       = [0,1, 0,1, 0,1];             % sliders forslicing
%isslice         = cutplanes;                   % coordinates for slicing 
dim             = [];
r_distance_from_profile = 50;                  % 
profile_ids     = [];                          % [id's; x-position, distances from rect] to create a 2D profile
profile_line    = [];
%
%
...
hTab_2d_viewer = uitab('v0','Parent',hTabGroup, 'Title','Model Inspector');
...
%% TAB-3, Panel-4: Inversion
    ...
    function CM_hAx_keep_and_spread_model(~,~,~)
        for i = 1:size(SURVEYS,1)
            if(i ~= data_1d_to_show)
                MDLS{i} = MDLS{data_1d_to_show};
            end
        end
    end
